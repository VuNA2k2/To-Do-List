...
class ProjectDetailBloc extends Bloc<ProjectDetailEvent, ProjectDetailState> {
  ...
  int _page = 0;
  bool _hasLoad = true;
  final GetTaskInProjectUseCase _getTaskInProjectUseCase =
  ...
  FutureOr<void> _initData(
    ...
    taskViewModels
        .addAll(pageRS.items.map(TaskMapper.getTaskViewModelFromTaskEntity));
    emit(ProjectDetailStableState(
      projectViewModel: event.projectViewModel,
  ...
  FutureOr<void> _loadMore(ProjectDetailLoadMoreEvent event,
    ...
    if (state is ProjectDetailLoadMoreState ||
        !_hasLoad ||
        state is ProjectDetailLoadingState) return;
    ...
    try {
      ...
      _hasLoad = pageRS.total == _limit;
      final List<TaskViewModel> taskViewModels =
          (state as ProjectDetailLoadMoreState).taskViewModels;
      taskViewModels
          .addAll(pageRS.items.map(TaskMapper.getTaskViewModelFromTaskEntity));
      emit(ProjectDetailStableState(
        projectViewModel:
  ...
  FutureOr<void> _deleteProject(
      ProjectDetailDeleteProjectEvent event, Emitter<ProjectDetailState> emit) {
